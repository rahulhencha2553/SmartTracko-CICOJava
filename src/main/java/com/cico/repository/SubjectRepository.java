...
//	@Query("SELECT s FROM Subject s LEFT JOIN FETCH s.chapters  as c WHERE s.subjectName = :subjectName AND s.isDeleted = 0 AND  c.isDeleted = 0")
//	Subject findBySubjectNameAndIsDeleted(@Param("subjectName") String subjectName);
//
//	@Query("SELECT s FROM Subject s LEFT JOIN FETCH s.chapters as  c WHERE s.subjectId = :subjectId AND  c.isDeleted = 0  ")
//	Optional<Subject> findBySubjectIdAndIsDeleted(@Param("subjectId") Integer subjectId);
//
//	@Query("SELECT  s FROM Subject s LEFT JOIN FETCH s.chapters as c WHERE  s.isDeleted = 0 AND  c.isDeleted = 0  ")
